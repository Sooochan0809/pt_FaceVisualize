(() => {
  ...
  function populateEmotionSelects() {
    const options = EMOTIONS.filter((emotion) => emotion !== BASE_EMOTION).map(
      (emotion) => {
  ...
  function updateLegend() {
    elements.targetLegend.textContent =
      EMOTION_LABELS[elements.targetEmotion.value];
  }
  ...
  function updateButtonStates() {
    ...
    const canNormalize =
      hasAnalysis &&
      videoReady &&
      anchors.peak > anchors.onset &&
      anchors.onset >= anchors.start;
  ...
  function seekVideo(time) {
    return new Promise((resolve) => {
      ...
      if (
        Math.abs(elements.video.currentTime - target) < 0.002 &&
  ...
  function updateBoundaryLog() {
    ...
    const settleText = includeSettle
      ? `${anchors.settle.toFixed(2)}s`
      : "未検出";
  ...
  function normalizedDuration() {
    return getSegments().reduce(
      (sum, segment) => sum + segment.targetDuration,
      0,
    );
  }
    ...
    for (const segment of segments) {
      ...
      if (normalizedTime <= end || segment === segments.at(-1)) {
        const progress = clamp(
          (normalizedTime - elapsed) / segment.targetDuration,
          0,
          1,
        );
        return (
          segment.sourceStart +
          (segment.sourceEnd - segment.sourceStart) * progress
        );
      }
      elapsed = end;
    }
  ...
  function getSmoothedSamples() {
    const windowSize = Math.max(
      1,
      Math.round(finiteNumber(elements.smoothWindow.value, 1)),
    );
  ...
  function drawChart() {
    ...
    const xFor = (time) =>
      CHART_PADDING.left +
      clamp(time / Math.max(duration, 0.001), 0, 1) * plotWidth;
    const yFor = (value) =>
      CHART_PADDING.top + (1 - clamp(value, 0, 1)) * plotHeight;
  ...
  function autoDetectBoundaries() {
    ...
    const crossingTime = (
      ...
      for (let index = startIndex; index <= endIndex; index += 1) {
        ...
        const ratio =
          Math.abs(valueDistance) < 1e-6
            ? 0
            : clamp((level - previous.value) / valueDistance, 0, 1);
        return (
          previous.sample.time +
          (current.sample.time - previous.sample.time) * ratio
        );
      }
      return null;
    };
  ...
  function updatePreviewUI() {
    const duration = normalizedDuration();
    elements.previewRange.value = String(
      clamp(previewPosition, 0, Math.max(duration, 0.001)),
    );
  ...
  function previewTick(now) {
    ...
    if (
      ...
      Math.abs(elements.video.currentTime - sourceTime) > 0.015
    ) {
  ...
  function playPreview() {
    ...
    elements.video.pause();
    if (previewPosition >= duration - 0.01) previewPosition = 0;
    previewPlaying = true;
  ...
  function safeBaseName() {
    return (sourceFileName.replace(/\.[^.]+$/, "") || "expression").replace(
      /[\\/:*?"<>|]+/g,
      "-",
    );
  }
  ...
  function exportJson() {
    const data = {
      ...
      segments: getSegments().map((segment) => ({
        ...
        playbackRate:
          (segment.sourceEnd - segment.sourceStart) / segment.targetDuration,
      })),
    };
    downloadBlob(
      new Blob([JSON.stringify(data, null, 2)], { type: "application/json" }),
      `${safeBaseName()}-normalization.json`,
    );
  }
  ...
  function bindEvents() {
    ...
    elements.previewButton.addEventListener("click", () =>
      previewPlaying ? pausePreview() : playPreview(),
    );
